...
class LuhnTest < Minitest::Test
  ...
  def test_initialize_raises_error_non_integer
    assert_raises(ArgumentError) { Luhn.new('hi') }
  end

  def test_initialize_raises_error_negative_int
    assert_raises(ArgumentError) { Luhn.new(-9) }
  end
end
